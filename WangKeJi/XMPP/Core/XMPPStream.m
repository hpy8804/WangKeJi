...
- (void)xmppParser:(XMPPParser *)sender didReadRoot:(NSXMLElement *)root
{
	...
	else
	{
		...
		else
		{
            ...
            NSXMLElement *query_username = [NSXMLElement elementWithName:@"username" xmlns:@"wzz"];
            NSXMLElement *query_password = [NSXMLElement elementWithName:@"password" xmlns:@"1234"];
			
            XMPPIQ *iq = [XMPPIQ iqWithType:@"get" elementID:[self generateUUID]];
            [iq addChild:query];
            [iq addChild:query_username];
            [iq addChild:query_password];
			
			NSString *outgoingStr = @"<iq xmlns=\"jabber:client\" type=\"get\" id=\"arPFa-21\"><query xmlns=\"jabber:iq:auth\"><username>wzz</username><password>1234</password></query></iq>";
			NSData *outgoingData = [outgoingStr dataUsingEncoding:NSUTF8StringEncoding];
